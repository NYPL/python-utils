...
def parse_research_catalog_identifier(identifier: str):
    ...
    if not isinstance(identifier, str):
        raise ResearchCatalogIdentifierError(f"Invalid RC identifier: {identifier}")
    ...
    return {"nyplSource": nyplSource, "nyplType": nyplType, "id": id}
...
def research_catalog_id_prefix(nyplSource: str, nyplType="bib"):
    ...
    if nypl_core_source_mapping().get(nyplSource) is None:
        raise ResearchCatalogIdentifierError(f"Invalid nyplSource: {nyplSource}")
...
def nypl_core_source_mapping():
    ...
    url = os.environ.get(
        "NYPL_CORE_SOURCE_MAPPING_URL",
        "https://raw.githubusercontent.com/NYPL/nypl-core/master/mappings/recap-discovery/nypl-source-mapping.json",
    )  # noqa
    try:
        ...
        raise ResearchCatalogIdentifierError(
            ...
            " {errorType} {errorMessage}".format(
                url=url, errorType=type(e), errorMessage=e
            )
        ) from None
    ...
    try:
        ...
        raise ResearchCatalogIdentifierError(
            ...
            " {errorMessage}".format(errorType=type(e), errorMessage=e)
        ) from None
