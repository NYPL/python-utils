...
class Oauth2ApiClient:
    ...
    def __init__(
        ...
        self.token_url = token_url or os.environ.get("NYPL_API_TOKEN_URL", None)
        self.base_url = base_url or os.environ.get("NYPL_API_BASE_URL", None)
    ...
    def get(self, request_path, **kwargs):
        ...
        try:
            ...
            self.logger.warning(
                f"Get request using path {request_path} \
...
Oauth2 Client using path: {request_path}. Retry #{retries}"
    ...
    def _do_http_method(self, method, request_path, **kwargs):
        ...
        try:
            ...
            kwargs_cleaned = {
                k: kwargs[k] for k in kwargs if not k.startswith("_do_http_method_")
            }
            ...
            if kwargs["_do_http_method_token_refreshes"] > 3:
                raise Oauth2ApiClientError("Exhausted token refreshes") from None
