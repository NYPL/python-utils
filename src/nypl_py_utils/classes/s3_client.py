...
class S3Client:
    ...
    def __init__(self, bucket, resource):
        ...
        try:
            ...
            self.logger.error("Could not create S3 client: {err}".format(err=e))
            raise S3ClientError(
    ...
    def fetch_cache(self):
        ...
        self.logger.info(
            "Fetching {file} from S3 bucket {bucket}".format(
                file=self.resource, bucket=self.bucket
            )
        )
        try:
            output_stream = BytesIO()
            self.s3_client.download_fileobj(self.bucket, self.resource, output_stream)
            return json.loads(output_stream.getvalue())
        except ClientError as e:
            self.logger.error(
                "Error retrieving {file} from S3 bucket {bucket}: {error}".format(
                    file=self.resource, bucket=self.bucket, error=e
                )
            )
            raise S3ClientError(
                "Error retrieving {file} from S3 bucket {bucket}: {error}".format(
    ...
    def set_cache(self, state):
        ...
        try:
            ...
            self.s3_client.upload_fileobj(input_stream, self.bucket, self.resource)
        except ClientError as e:
            self.logger.error(
                "Error uploading {file} to S3 bucket {bucket}: {error}".format(
                    file=self.resource, bucket=self.bucket, error=e
                )
            )
            raise S3ClientError(
