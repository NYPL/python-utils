        ...
        "schema": json.dumps(
            {
                ...
                "type": "record",
                "fields": [
                    {"name": "patron_id", "type": "int"},
                    {"name": "library_branch", "type": ["null", "string"]},
                ],
            }
        )
    }
...
class TestAvroClient:
    ...
    def test_avro_encoder_instance(self, requests_mock):
        requests_mock.get("https://test_schema_url", text=json.dumps(_TEST_SCHEMA))
    ...
    def test_get_json_schema(
        self, test_avro_encoder_instance, test_avro_decoder_instance
    ):
    ...
    def test_bad_json_error(self, requests_mock):
        requests_mock.get("https://test_schema_url", text="bad json")
        with pytest.raises(AvroClientError):
            AvroEncoder("https://test_schema_url")
    ...
    def test_encode_batch(self, test_avro_encoder_instance, test_avro_decoder_instance):
        ...
        for i in range(3):
            ...
            assert (
                test_avro_decoder_instance.decode_record(encoded_records[i])
                == TEST_BATCH[i]
            )
    ...
    def test_decode_record_b64(self, test_avro_decoder_instance):
        ...
        assert (
            test_avro_decoder_instance.decode_record(TEST_ENCODED_RECORD, "base64")
            == TEST_DECODED_RECORD
        )
