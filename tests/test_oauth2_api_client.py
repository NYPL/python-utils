...
from nypl_py_utils.classes.oauth2_api_client import (
    Oauth2ApiClient,
...
class TestOauth2ApiClient:
    ...
    def test_generate_access_token(self, test_instance, token_server_post):
        ...
        assert (
            test_instance.oauth_client.token["access_token"]
    ...
    def test_do_http_method(self, requests_mock, token_server_post, test_instance):
    ...
    def test_token_expiration(
        ...
        api_get_mock = requests_mock.get(f"{BASE_URL}/foo", json={"foo": "bar"})
        ...
        assert (
            api_get_mock.request_history[0]._request.headers["Authorization"]
        ...
        second_token_server_post = requests_mock.post(
            TOKEN_URL, text=json.dumps(second_token_response)
        )
        ...
        assert (
            api_get_mock.request_history[1]._request.headers["Authorization"]
            == "Bearer super-secret-second-token"
        )

    def test_error_status_raises_error(
    ...
    def test_bad_response_no_retries(self, requests_mock, test_instance, mocker):
        mocker.patch.object(
            test_instance, "_do_http_method", return_value=MockEmptyResponse(empty=True)
        )
    ...
    def test_http_retry_fail(self, requests_mock, test_instance_with_retries, mocker):
        mocker.patch.object(
        ...
        assert (
            resp.message
            == "Oauth2 Client: Request failed after 3 \
                            empty responses received from Oauth2 Client"
        )

    def test_http_retry_success(
        self, requests_mock, test_instance_with_retries, mocker
    ):
        mocker.patch.object(
            test_instance_with_retries,
            "_do_http_method",
            side_effect=[
                MockEmptyResponse(empty=True),
