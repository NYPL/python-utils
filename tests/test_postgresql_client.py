...
class TestPostgreSQLClient:
    ...
    def test_execute_write_query_with_params(self, mock_pg_conn, test_instance, mocker):
        test_instance.connect()
        ...
        mock_cursor.execute.assert_called_once_with("test query %s %s", ("a", 1))
        test_instance.conn.commit.assert_called_once()
        mock_cursor.close.assert_called_once()

    def test_execute_query_with_exception(self, mock_pg_conn, test_instance, mocker):
        test_instance.connect()
