...
class TestPostgreSQLPoolClient:
    ...
    def test_instance(self, mocker):
        ...
        return PostgreSQLPoolClient(
            "test_host", "test_port", "test_db_name", "test_user", "test_password"
        )

    def test_init(self, test_instance):
        assert test_instance.pool.conninfo == (
            "postgresql://test_user:test_password@test_host:test_port/" + "test_db_name"
        )
    ...
    def test_connect(self, test_instance):
        ...
        test_instance.pool.open.assert_called_once_with(wait=True, timeout=300.0)

    def test_connect_with_exception(self, mocker):
        mocker.patch("psycopg_pool.ConnectionPool.open", side_effect=Error())
    ...
    def test_execute_read_query(self, test_instance, mocker):
        ...
        mocker.patch(
            "psycopg_pool.ConnectionPool.connection", return_value=mock_conn_context
        )

        assert test_instance.execute_query("test query") == [(1, 2, 3), ("a", "b", "c")]
    ...
    def test_execute_write_query_with_params(self, test_instance, mocker):
        ...
        assert (
            test_instance.execute_query("test query %s %s", query_params=("a", 1))
            is None
        )
        mock_conn.execute.assert_called_once_with("test query %s %s", ("a", 1))
    ...
    def test_reopen_pool(self, test_instance, mocker):
        ...
        test_instance.pool.open.assert_has_calls(
            [mocker.call(wait=True, timeout=300), mocker.call(wait=True, timeout=300)]
        )
